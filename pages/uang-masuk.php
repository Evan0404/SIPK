<div class="row">
    <div class="col-md-7 mb-3">
        <div class="card shadow w-100 p-2 pt-3" id="add">
            <h6 class="mb-2">Tambah Uang Masuk</h6>
            <form method="post">
                ...
                <div class="form-group">
                    ...
                    <select name="kategori" id="select" class="form-control">
                        ...
                        <?php
                            $getKategori=$con->query("SELECT * FROM kategori WHERE status = 'Uang Masuk'");
    ...
    <?php
        if(isset($_POST['tambah'])){
            $con->query("INSERT INTO masuk_keluar (judul, kategori, jumlah, tgl, keterangan, user_id, jenis) VALUES ('$_POST[judul]', '$_POST[kategori]', '$_POST[jumlah]', '$_POST[tgl]', '$_POST[keterangan]', '1', 'Masuk')");
            echo"
                <script>
                    alert('Berhasil Menambah Uang Masuk');
                    document.location.href='index.php?hal=uang-masuk';
                </script>
            ";
        }
        ...
        if(isset($_POST['ubah'])){
            ...
            echo"
                <script>
                    alert('Berhasil Mengubah Uang Masuk');
                    document.location.href='index.php?hal=uang-masuk';
                </script>
            ";
        }
        ...
        if(isset($_GET['del'])){
            ...
            echo"
                <script>
                    alert('Berhasil Menghapus Uang Masuk');
                    document.location.href='index.php?hal=uang-masuk';
                </script>
            ";
        }
    ...
    <div class="col-md-5">
        <div class="card shadow w-100 p-2 pt-3 h-100 " style="max-height:100%; overflow-y: scroll;">
            <h6 class="mb-2">Daftar Uang Masuk</h6>
            <?php
            ...
            <div class="alert alert-<?= $alertC?> text-light" role="alert">
                <p class="m-0" align="left">
                    <b>Rp <?= number_format($totalMasuk['total'],0,'','.')?></b>
                </p>
            </div>
            <br>
            <div class="table-responsive">
                <table class="table" id="myTable">
                    ...
                    <tbody>
                        <?php
                            $getMasuk = $con->query("SELECT * FROM masuk_keluar WHERE jenis = 'Masuk' ORDER BY tgl DESC");
                                ...
                                <td class="align-middle">
                                    <span class="text-secondary text-xs font-weight-bold">
                                        <div class="btn-group" role="group" aria-label="Basic example">
                                            ...
                                            <a href="index.php?hal=uang-keluar&del=<?= $data['id_masuk_keluar']?>" onclick="return confirm('Data yang dihapus tidak dapat kembali, apakah anda yakin ingin menghapus data ini ?')" class="btn btn-danger btn-sm"><center><i class="bi bi-trash"></center></i></a>
                                        </div>
                                    </span>
                                </td>        
                            </tr>
